...
impl Etymology {
    pub fn new(word: &str) -> Result<Etymology> {
        ...
        let label = Etymology::extract_word_name(&results_html).unwrap();
        Ok(Etymology {
            word: word.to_owned(),
            label,
            etymology,
    ...
    pub fn extract_word_name(raw_html: &str) -> Option<String> {
        // TODO: Should return a Result, in case no hits were found.
        let d = Html::parse_document(raw_html);
